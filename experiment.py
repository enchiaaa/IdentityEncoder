...
class LitModel(pl.LightningModule):
    ...
    def infer_whole_dataset(self,
        ...
        if with_render:
            ...
            render_save_dir = render_save_path or "./rendered_images"
        ...
        for batch in tqdm(loader, total=len(loader), desc='infer'):
            with torch.no_grad():
                ...
                if with_render:
                    ...
                    render = (render + 1) / 2  # 正規化到 [0, 1]
                    render = self.all_gather(render)
                    if render.dim() == 5:
                        render = render.flatten(0, 1)

                    if self.global_rank == 0:
                        for i, img in enumerate(render[argsort]):
                            save_path = os.path.join(render_save_dir, f"image_{idx[argsort][i].item()}.png")
